...
repDir <- paste0(mainDir, "reports/1.readData_rep1/")
...
#sampleinfo <- read.table(paste0(mainDir, "reports/res/1.Check_download/dsample.info.csv"), sep=";", header=T, stringsAsFactors=FALSE)

#save(sampleinfo, file=paste0(dataDir, "sampleinfo.RData"))
#write.table(sampleinfo, file=paste0(tabDir, "sampleinfo.xls"), sep="\t", quote=F)
#hwrite(sampleinfo, page=paste0(repDir, "sampleinfo.html"))


#### Read matrix format

barcode.path <- paste0(dataDir, "seq_rep1/barcodes.tsv")
features.path <- paste0(dataDir, "seq_rep1/features.tsv")
matrix.path <- paste0(dataDir, "seq_rep1/matrix.mtx")
...
pdf(paste0(figDir, "maxID.ridge.pdf"), width=25, height=5)
for (i in names(seus)) {
    seu <- seus[[i]]
...
pdf(paste0(figDir, "hash.ID.ridge.pdf"), width=25, height=8)
for (i in names(seus)) {
    seu <- seus[[i]]
    Idents(seu) <- "hash.ID"
    print(RidgePlot(seu, assay = "hash", features = rownames(seu[["hash"]])[1:4], ncol = 4))
...
pdf(paste0(figDir, "hash_classification.ridge.pdf"), width=55, height=8)
for (i in names(seus)) {
    seu <- seus[[i]]
...
save(seus, file=paste0(repDir, "seus_r1.RData"))
